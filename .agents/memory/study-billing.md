...
## Ambassador residual commissions: cross-channel idempotency + clawback mapping
Commission events dedupe on a unique `(sourceKind, sourcePaymentId)`. Local/mobile credits use `sourceKind:"local"` (the `study_payments.id`) and are minted inside `activatePayment`'s tx; card credits use `sourceKind:"stripe"` (the Stripe **invoice id**) and are minted only from the `invoice.payment_succeeded` webhook.
**Why:** the dedupe key is only safe because one underlying payment is never represented by both kinds. It is NOT a universal cross-channel key, so never start crediting the same real payment from both the local path and the Stripe path.
**How to apply:** Stripe clawback must target the SAME source id that minted the event (the invoice id). `charge.refunded`'s event object is a charge and exposes `invoice` directly; `charge.dispute.created`'s object is a **dispute** with NO `invoice` field, so you must retrieve the charge (`getUncachableStripeClient().charges.retrieve(dispute.charge)`) to read its invoice id before calling `clawbackBySourcePayment`. (Stripe types: cast the retrieved charge to read `invoice`; the SDK's `Response<Charge>` wrapper trips tsc.)

## Ambassador cash-out must lock the ambassador row, not just re-derive in a tx
`requestCashout` derives available = confirmed_commissions - committed_payouts and inserts a payout. A plain tx is NOT enough: two concurrent requests read the same balance and both insert, over-drawing.
**Why:** balances are ledger-derived (no stored balance column to constrain), so the only serialization point is the ambassador row itself.
**How to apply:** `SELECT ... FOR UPDATE` the ambassador row as the first statement inside the tx (`.for("update")` in Drizzle), then do all balance reads + the payout insert. Cashable is floored to `cashoutIncrementUsdMinor` (default $20). Post-payout clawback can drive available negative; that is represented and intended.
